...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_69_ansi(
        ...
        KC_ESC,  KC_LOPTN, KC_LCMMD,          KC_SPC,           EISU_FN2, KANA_FN2,       KC_SPC,            KC_TEAM_MUTE,       KC_LEFT, KC_DOWN, KC_RGHT),
    ...
    [WIN_BASE] = LAYOUT_69_ansi(
        ...
        KC_LCTL, KC_A,     KC_S,     KC_D,    KC_F,    KC_G,              KC_H,    KC_J,    KC_K,    KC_L,     KC_SCLN,  KC_QUOT,  KC_ENT,           KC_DEL,
        KC_LSFT,           KC_Z,     KC_X,    KC_C,    KC_V,    KC_B,     KC_B,    KC_N,    KC_M,    KC_COMM,  KC_DOT,   KC_SLSH,  KC_RSFT, KC_UP,
        KC_ESC,  KC_LWIN,  KC_LALT,           KC_SPC,           EISU_FN2, KANA_FN2,       KC_SPC,            KC_TEAM_MUTE,       KC_LEFT, KC_DOWN, KC_RGHT),
    ...
    [_NICOLA] = LAYOUT_69_ansi(
        ...
        KC_ESC,  KC_TRNS,  KC_TRNS,           NG_SHFTL,         EISU_FN2, KANA_FN2,       NG_SHFTR,          KC_TEAM_MUTE,       KC_LEFT, KC_DOWN, KC_RGHT),

    [MAC_FN1] = LAYOUT_69_ansi(
        KC_TILD, KC_F1,    KC_F2,    KC_F3,   KC_F4,   KC_F5,   KC_F6,    KC_F7,      KC_F8,   KC_F9,   KC_F10,   KC_F11,   KC_F12,   _______,          BL_TOGG,
        KC_KANA, TG_JP,    KC_HOME,  KC_UP,   KC_END,  _______, _______,  _______,    _______, _______,  _______,  KC_PSCR,  _______,  _______,          _______,
    ...
    [FN2] = LAYOUT_69_ansi(
        ...
        _______,           KC_VOLD,  BT_HST1, BT_HST2, BT_HST3, BAT_LVL,  BAT_LVL,    KC_DEL,  KC_END,  KC_PGDN,  _______,  _______,  _______, BL_UP,
...
static uint16_t eisu_fn2_timer;
static uint16_t kana_fn2_timer;
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    if (record->event.pressed) {
        switch (keycode) {
            case EISU_FN2:
                eisu_fn2_timer = timer_read();
                layer_on(FN2);
                return false;
            case KANA_FN2:
                kana_fn2_timer = timer_read();
                layer_on(FN2);
                return false;
        ...
        switch (keycode) {
            case EISU_FN2:
                layer_off(FN2);
                if (timer_elapsed(eisu_fn2_timer) < TAPPING_TERM) {
                    layer_off(_NICOLA);
                    nicola_off();
                    tap_code(KC_LNG2);
                }
            ...
            case KANA_FN2:
                layer_off(FN2);
                if (timer_elapsed(kana_fn2_timer) < TAPPING_TERM) {
                    if (nicola_state()) {
                        layer_off(_NICOLA);
                        nicola_off();
                        tap_code(KC_LNG2); // IME OFF
